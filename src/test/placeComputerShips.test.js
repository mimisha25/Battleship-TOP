...
beforeEach(() => {
    ...
    canPlaceComputerShip.mockClear();
    setComputerShips.mockClear();
});
...
describe('placeComputerShips', () => {
    ...
    test('should correctly place ships with different orientations', () => {
        ...
        placedShips.forEach(ship => {
            if (ship.orientation === 'horizontal') expect(ship.coordinates[0].row).toBe(ship.coordinates[ship.coordinates.length - 1].row);
            else if (ship.orientation === 'vertical') expect(ship.coordinates[0].col).toBe(ship.coordinates[ship.coordinates.length - 1].col);
        });
    });

    test('should handle a case where canPlaceComputerShip fails', () => {
        canPlaceComputerShip.mockReturnValueOnce(false).mockReturnValue(true);
        const mockCell = { classList: { add: jest.fn() } };
        document.querySelector.mockReturnValue(mockCell);
        placeComputerShips();
        expect(canPlaceComputerShip).toHaveBeenCalledTimes(6);
        expect(mockCell.classList.add).toHaveBeenCalledWith('ship-placement');
    });
});